...
ACA and Medicaid expansion are the two policies that I can think of. Since theses policies makes healthcare more accessible for people, the direct purchase health insurance in later years can experience a reduction. 
...
m.dd <- feols(perc_unins ~ treat | State + year, data = reg.dat)
...
summary(m.dd)
```
The result is different but not sure if it is significantly different. The major reason behind the difference is that the number of observations are different. 
